        ...
        {isLoading && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-[220px] rounded-2xl bg-white/5 animate-pulse border border-white/5"
              />
            ))}
          </div>
        )}
            ...
            {collections.map((collection) => {
              ...
              return (
                <motion.div key={collection.id} variants={itemVariants}>
                  <GlassCard
                    interactive
                    className="p-6 group cursor-pointer h-full border border-white/5 hover:border-accent-blue/30 transition-all duration-300"
                  >
                    <div
                      className={`p-4 rounded-xl bg-gradient-to-br ${bgColor} opacity-80 mb-4 w-fit group-hover:opacity-100 transition-all duration-300 shadow-lg shadow-accent-blue/20`}
                    >
        ...
        {!isLoading && collections.length === 0 && !isError && (
          <div className="text-center py-16 px-4 rounded-2xl border border-white/5 bg-white/[0.02]">
            ...
            <p className="text-muted-foreground max-w-sm mx-auto mb-6">
              Collections help you group your prompts by project, workflow, or
              client.
            </p>
            <Button size="lg">

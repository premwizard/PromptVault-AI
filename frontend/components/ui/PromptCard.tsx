      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-start justify-between mb-4">
            ...
            >
              <Heart
                className={cn('w-5 h-5 transition-colors', isFavorited ? 'fill-red-500 text-red-500' : 'text-muted-foreground hover:text-red-500')}
              />
            </motion.button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="inline-block px-2 py-1 text-xs rounded-full bg-white/5 text-muted-foreground border border-white/10"
              >
                #{tag}
              </span>
            ))}
            {tags.length > 3 && (
              <span className="text-xs text-muted-foreground py-1">+{tags.length - 3}</span>
            )}
          </div>
